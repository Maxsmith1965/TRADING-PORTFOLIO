...
// Reads and writes stock data to Netlify Blobs
// GET /.netlify/functions/stocks - returns all stocks
// POST /.netlify/functions/stocks - saves a stock
// DELETE /.netlify/functions/stocks?ticker=NVDA - removes a stock

const { getStore } = require('@netlify/blobs');

const STORE_NAME = 'maxsmith-stocks';
...
// Default stock database - used if Blobs is empty
const DEFAULT_STOCKS = {
  ...
  activeStocks: [
    ...
    { ticker:'CEG', name:'Constellation Energy', pot:'active', alloc:12, beer:60, froth:40, risk:'MEDIUM', note:'Nuclear power — essentially fair value', sector:'Nuclear Power', earnings:'2026-07-30', intrinsic:258 },
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      try {
        const data = await store.get('stocks', { type: 'json' });
        if (data) {
          return { statusCode: 200, headers, body: JSON.stringify(data) };
        }
      } catch(e) {
        // Blobs empty - seed with defaults
      }
      // Seed the database with defaults
      await store.setJSON('stocks', DEFAULT_STOCKS);
    ...
    if (event.httpMethod === 'POST') {
      ...
      let data;
      try {
        data = await store.get('stocks', { type: 'json' });
      } catch(e) {
        data = DEFAULT_STOCKS;
      }
      if (!data) data = DEFAULT_STOCKS;

      const pot = stock.pot === 'blue' ? 'blueChips' : 'activeStocks';
      const existing = data[pot].findIndex(s => s.ticker === stock.ticker.toUpperCase());

      if (existing >= 0) {
        data[pot][existing] = { ...data[pot][existing], ...stock };
      } else {
        data[pot].push(stock);
      }

      await store.setJSON('stocks', data);
    ...
    console.error('Stocks function error:', e);
